#!/usr/bin/python3

import os
import shutil
import subprocess
import time
import json
import threading
import configparser
from colorama import Fore, Back, Style, init
import requests
from simple_term_menu import TerminalMenu
import pyfiglet
from thefuzz import process, fuzz
from bs4 import BeautifulSoup

SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
CONFIG_PATH = os.path.join(SCRIPT_DIR, "config.ini")


def clear_screen():
    os.system("cls" if os.name == "nt" else "clear")


def rd_add_and_download(magnetlink, headers, download_path, background=False):
    """Shared helper: add magnet to Real-Debrid, select files, unrestrict links, download."""
    try:
        apiurl = "https://api.real-debrid.com/rest/1.0/torrents/addMagnet"
        payload = {"magnet": magnetlink}
        response = requests.post(apiurl, data=payload, headers=headers)

        print("RD addMagnet status:", response.status_code)
        print("RD addMagnet body:", response.text)

        try:
            data = response.json()
        except (ValueError, json.JSONDecodeError):
            print(f"{Fore.RED}Non-JSON response from Real-Debrid{Style.RESET_ALL}")
            return False

        if response.status_code not in (200, 201) or "id" not in data:
            print(f"{Fore.RED}Real-Debrid error: {data}{Style.RESET_ALL}")
            return False

        torrentId = data["id"]
        print("Real-Debrid torrentId:", torrentId)

        apiurl2 = f"https://api.real-debrid.com/rest/1.0/torrents/selectFiles/{torrentId}"
        fileSelect = {"files": "all"}
        selectFilesResponse = requests.post(apiurl2, data=fileSelect, headers=headers)
        print(selectFilesResponse.text)

        apiurl3 = f"https://api.real-debrid.com/rest/1.0/torrents/info/{torrentId}"

        # Poll until Real-Debrid has finished fetching the torrent
        max_wait = 120  # seconds
        poll_interval = 5
        waited = 0
        getLink = None

        while waited < max_wait:
            torrentDetails = requests.get(apiurl3, headers=headers)

            try:
                getLink = torrentDetails.json()
            except (ValueError, json.JSONDecodeError):
                print(f"{Fore.RED}Non-JSON response from torrent info{Style.RESET_ALL}")
                return False

            status = getLink.get("status", "")
            print(f"RD torrent status: {status} (waited {waited}s)")

            if status == "downloaded":
                break
            elif status in ("magnet_error", "error", "virus", "dead"):
                print(f"{Fore.RED}Real-Debrid torrent failed: {status}{Style.RESET_ALL}")
                return False

            time.sleep(poll_interval)
            waited += poll_interval

        if getLink is None or getLink.get("status") != "downloaded":
            print(f"{Fore.RED}Timed out waiting for Real-Debrid to download torrent ({max_wait}s){Style.RESET_ALL}")
            return False

        linkToUnrestrict = getLink.get("links", [])
        if not linkToUnrestrict:
            print(f"{Fore.RED}No links found in torrent info{Style.RESET_ALL}")
            return False

        print(linkToUnrestrict)

        for i in linkToUnrestrict:
            try:
                apiurl4 = "https://api.real-debrid.com/rest/1.0/unrestrict/link"
                payload2 = {"link": i}
                unrestrictionData = requests.post(apiurl4, data=payload2, headers=headers)

                try:
                    linkDownloadGet = unrestrictionData.json()
                except (ValueError, json.JSONDecodeError):
                    print(f"{Fore.RED}Non-JSON response from unrestrict{Style.RESET_ALL}")
                    continue

                downloadLink = linkDownloadGet.get("download")
                if not downloadLink:
                    print(f"{Fore.RED}No download link in unrestrict response{Style.RESET_ALL}")
                    continue

                print(f"{Fore.GREEN}Starting download: {downloadLink}{Style.RESET_ALL}")
                if background:
                    subprocess.Popen(["wget", "-q", "-c", "-P", download_path, downloadLink],
                                     stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
                else:
                    subprocess.run(["wget", "-P", download_path, downloadLink])
                time.sleep(0.5)
            except requests.exceptions.RequestException as e:
                print(f"{Fore.RED}Network error during unrestrict: {e}{Style.RESET_ALL}")

        return True

    except requests.exceptions.RequestException as e:
        print(f"{Fore.RED}Network error: {e}{Style.RESET_ALL}")
        return False


def setup():
    print("Welcome to setup...")
    realdebridapikey = input("Real Debrid Api Key: ")
    config["CONFIG"]["REAL_DEBRID_API_KEY"] = realdebridapikey
    with open(CONFIG_PATH, "w") as configfile:
        config.write(configfile)

    omdbapikey = input("OMDB Api Key: ")
    config["CONFIG"]["OMDB_API_KEY"] = omdbapikey
    with open(CONFIG_PATH, "w") as configfile:
        config.write(configfile)

    print("Path Example: /home/user/media")
    mediapath = input("Media Path (This will create a movies and tvshow directory): ")
    os.makedirs(f"{mediapath}/movies", exist_ok=True)
    os.makedirs(f"{mediapath}/tvshow", exist_ok=True)
    config["CONFIG"]["MEDIA_PATH"] = mediapath
    with open(CONFIG_PATH, "w") as configfile:
        config.write(configfile)

    config["CONFIG"]["FIRST_LAUNCH"] = "1"
    with open(CONFIG_PATH, "w") as configfile:
        config.write(configfile)


config = configparser.ConfigParser()
config.read(CONFIG_PATH)

if not config.has_section("CONFIG") or not config.has_option("CONFIG", "FIRST_LAUNCH"):
    config["CONFIG"] = {"FIRST_LAUNCH": "0", "REAL_DEBRID_API_KEY": "", "OMDB_API_KEY": "", "MEDIA_PATH": ""}
    with open(CONFIG_PATH, "w") as configfile:
        config.write(configfile)

FIRST_LAUNCH = config["CONFIG"]["FIRST_LAUNCH"]

if FIRST_LAUNCH == "0":
    setup()

# Always read config after setup to ensure variables are defined
apikey = config["CONFIG"]["REAL_DEBRID_API_KEY"]
apikey2 = config["CONFIG"]["REAL_DEBRID_API_KEY"]
omdbapikey = config["CONFIG"]["OMDB_API_KEY"]
mediapath = config["CONFIG"]["MEDIA_PATH"]


def moviesearch():
    while True:
        movies = os.listdir(f"{mediapath}/movies/")
        count = 0
        print("| q | To go back to menu")
        print("| addmovie | to add movie")

        query = input("Which movie?: ")

        if query == "q":
            clear_screen()
            return
        elif query == "addmovie":
            clear_screen()
            scrape()
            return

        movienames = []
        for movie in movies:
            score = fuzz.partial_ratio(movie, query)
            if score > 60:
                print(f"{Fore.GREEN}Match Found{Style.RESET_ALL}")
                count = count + 1
                print(score)
                movienames.append(movie)
            else:
                print(f"{Fore.RED}Not a match{Style.RESET_ALL}")
        print(f"Amount of matches: {count}")
        for name in movienames:
            print(name)
        input("Press ENTER to continue...")
        clear_screen()


def scrape():
    while True:
        options = []
        print("| search | search for if movie exists")
        print("| q | Go back to main menu")
        search = input("Input search term: ")

        if search == "q":
            clear_screen()
            return

        elif search == "search":
            moviesearch()
            continue

        background = input("Send to background? (y) (n): ")
        sendtoback = background == "y"

        searchparse = search.replace(" ", "%20").replace("-b", "")

        pagecount = 1
        start_time = time.perf_counter()

        def timenow(message):
            current_time = time.perf_counter()
            final_time = current_time - start_time
            print(f"[{final_time:.2f}] {message}")

        coun5 = 1
        options_lock = threading.Lock()

        def getdata(pagecount):
            url = f"https://rargb.to/search/{pagecount}/?search={searchparse}&category[]=movies"
            timenow("Starting Search Request...")

            try:
                response = requests.get(url)
            except requests.exceptions.RequestException as e:
                timenow(f"{Fore.RED}Request failed: {e}{Style.RESET_ALL}")
                return

            if response.status_code == 200:
                timenow(f"{Fore.GREEN}Status Code 200 OK{Style.RESET_ALL}")
            else:
                timenow(f"{Fore.RED}Error occured (status {response.status_code}){Style.RESET_ALL}")
                return

            htmlresponse = response.text
            soup = BeautifulSoup(htmlresponse, 'html.parser')
            atags = soup.find_all('a')

            blacklist = {
                "/torrent/the-eagle-2011-1080p-bluray-x265-hevc-10bit-aac-5-1-tigole-qxr-5294715.html",
                "/torrent/mommys-little-star-2022-720p-web-dl-aac2-0-h264-lbr-5296687.html",
                "/torrent/the-bridge-on-the-river-kwai-1957-1080p-bluray-1600mb-dd2-0-x264-galaxyrg-5296661.html",
                "/torrent/color-my-world-with-love-2022-hallmark-720p-webrip-hevc-poke-5295948.html",
                "/torrent/the-godfather-1972-rm4k-repack-1080p-bluray-x265-hevc-10bit-aac-5-1-tigole-qxr-5297358.html",
                "/torrent/the-good-neighbor-2022-1080p-webrip-dd5-1-x-264-evo-5294301.html",
                "/torrent/firestarter-2022-720p-bluray-800mb-x264-galaxyrg-5294643.html",
                "/torrent/the-winston-affair-1964-robert-mitchum-war-1080p-brrip-x264-classics-5296579.html"
            }

            for tag in atags:
                href = tag.get('href')
                if not href or "/torrent/" not in href:
                    continue
                if "4k" in href:
                    timenow(f"{Fore.RED}Has 4k in name REJECTED{Style.RESET_ALL}")
                elif "2160" in href:
                    timenow(f"{Fore.RED}Has 2160 in name REJECTED{Style.RESET_ALL}")
                elif href in blacklist:
                    pass
                else:
                    with options_lock:
                        options.append(href)
                    print(href)
                    print(pagecount)

        threadlist2 = []

        while coun5 != 4:
            thread = threading.Thread(target=getdata, args=(pagecount,))
            threadlist2.append(thread)
            coun5 += 1
            pagecount += 1
            time.sleep(0.1)
            timenow("Adding thread to list")

        for t in threadlist2:
            t.start()
            timenow(f"{Fore.GREEN}Starting {t}{Style.RESET_ALL}")

        timenow(f"{Fore.GREEN}Waiting for threads to finish... {Style.RESET_ALL}")

        for t in threadlist2:
            t.join()

        clear_screen()
        timenow(f"{Fore.GREEN}Threads finished{Style.RESET_ALL}")

        options_set = set(options)
        options.clear()
        options.extend(options_set)

        siteforlink = []
        count = 0
        baseurl = "https://rargb.to"

        torrent_info = {}
        info_lock = threading.Lock()

        def gathermagnetlinks(pathtotorrent):
            correctedurl = f"{baseurl}{pathtotorrent}"
            timenow(f"{Fore.GREEN}Generated URL {correctedurl}{Style.RESET_ALL}")

            siteforlink.append(correctedurl)
            timenow(f"Sending request to {correctedurl}")

            try:
                magnetlinkresponse = requests.get(correctedurl)
            except requests.exceptions.RequestException as e:
                timenow(f"{Fore.RED}Request failed for {correctedurl}: {e}{Style.RESET_ALL}")
                return

            timenow(f"{Fore.GREEN}Recieved response from request{Style.RESET_ALL}")

            parse = BeautifulSoup(magnetlinkresponse.text, 'html.parser')

            title = "Unknown title"
            h1 = parse.find("h1")
            if h1:
                title = h1.get_text(strip=True)

            magnet = None
            for a in parse.find_all('a'):
                href2 = a.get('href')
                if href2 and "magnet" in href2:
                    magnet = href2
                    break

            size = "Unknown"
            for tr in parse.find_all("tr"):
                tds = tr.find_all("td")
                if len(tds) == 2 and "Size" in tds[0].get_text():
                    size = tds[1].get_text(strip=True)
                    break
            if magnet:
                with info_lock:
                    torrent_info[pathtotorrent] = {
                        "title": title,
                        "size": size,
                        "magnet": magnet
                    }
                timenow(f"{Fore.GREEN}Stored: {title} ({size}){Style.RESET_ALL}")
            else:
                timenow(f"{Fore.RED}NO magnet found for {correctedurl}{Style.RESET_ALL}")

        threadlist = []
        count = 0

        for pathtotorrent in options:
            thread = threading.Thread(target=gathermagnetlinks, args=(pathtotorrent,))
            threadlist.append(thread)

        threadcount = len(threadlist)

        for t in threadlist:
            t.start()
            count += 1
            timenow(f"{Fore.GREEN}Initializing Thread{Style.RESET_ALL}")
            timenow(f"{count}/{threadcount}")

        timenow("Waiting for threads to finish")

        for t in threadlist:
            t.join()

        clear_screen()
        timenow(f"{Fore.GREEN}Threads Finished{Style.RESET_ALL}")

        optionsparsed = ["[q] Quit", "[r] Restart search"]
        count = 0
        optionscount = len(options)

        paired_results = []

        for path in options:
            count += 1
            timenow(f"Parsing options {Fore.GREEN}{count}/{optionscount}{Style.RESET_ALL} ")

            if path in torrent_info:
                info = torrent_info[path]
                title = info["title"]
                size = info["size"]
                magnet = info["magnet"]
                label = f"{title}  ({size})"
                paired_results.append((label, magnet))
                optionsparsed.append(label)
        clear_screen()

        current_timern = time.perf_counter()
        timenownow = current_timern - start_time
        print(f"{Fore.GREEN}Took {timenownow:.2f}s{Style.RESET_ALL}")

        terminal_menu = TerminalMenu(optionsparsed, title="CHOOSE")
        menu_entry = terminal_menu.show()

        if menu_entry is None:
            clear_screen()
            return

        if optionsparsed[menu_entry] == "[q] Quit":
            print(f"{Fore.RED}Quitting...{Style.RESET_ALL}")
            time.sleep(0.5)
            clear_screen()
            return

        elif optionsparsed[menu_entry] == "[r] Restart search":
            print("Restarting...")
            time.sleep(0.5)
            clear_screen()
            continue

        else:
            selected_index = menu_entry - 2
            if selected_index < 0 or selected_index >= len(paired_results):
                print(f"{Fore.RED}Invalid selection{Style.RESET_ALL}")
                time.sleep(1)
                continue

            title, magnet = paired_results[selected_index]

            headers = {
                "Authorization": f"Bearer {apikey}",
                "Accept": "application/json"
            }

            rd_add_and_download(magnet, headers, f"{mediapath}/movies/", background=sendtoback)
            clear_screen()
            continue


def tvshow():
    while True:
        clear_screen()
        dirs = os.listdir(f"{mediapath}/tvshow/")
        tvoptions = ["[a] OMDB Automation", "[n] Add New", "[q] Quit"]

        for dir in dirs:
            tvoptions.append(f"{dir}")

        tvshowmenu = TerminalMenu(tvoptions)
        menu_entry = tvshowmenu.show()

        if menu_entry is None:
            clear_screen()
            return

        if tvoptions[menu_entry] == "[q] Quit":
            clear_screen()
            return

        if tvoptions[menu_entry] == "[a] OMDB Automation":
            while True:
                clear_screen()
                movieName = input("TV Show Name (q to go back): ")
                if movieName == "q":
                    break

                movieNameParsed = movieName.replace(" ", "+")
                clear_screen()

                try:
                    response = requests.get(f"http://www.omdbapi.com/?t={movieNameParsed}&apikey={omdbapikey}")
                    data = response.json()
                except requests.exceptions.RequestException as e:
                    print(f"{Fore.RED}Network error: {e}{Style.RESET_ALL}")
                    time.sleep(3)
                    continue
                except (ValueError, json.JSONDecodeError):
                    print(f"{Fore.RED}Invalid response from OMDB{Style.RESET_ALL}")
                    time.sleep(3)
                    continue

                print("Received JSON data:")
                print(json.dumps(data, indent=2))

                if data.get("Response") == "False":
                    print(f"{Fore.RED}Show not found: {data.get('Error', 'Unknown error')}{Style.RESET_ALL}")
                    time.sleep(3)
                    continue

                totalSeasons = data.get("totalSeasons")
                showTitle = data.get("Title")

                if not totalSeasons or not showTitle:
                    print(f"{Fore.RED}Missing season/title data from OMDB{Style.RESET_ALL}")
                    time.sleep(3)
                    continue

                print(showTitle)
                print(totalSeasons)

                showTitleParsed = showTitle.replace(" ", "_")
                print(showTitleParsed)

                totalSeasonsInt = int(totalSeasons)

                os.makedirs(f"{mediapath}/tvshow/{showTitleParsed}", exist_ok=True)

                while totalSeasonsInt != 0:
                    os.makedirs(f"{mediapath}/tvshow/{showTitleParsed}/Season_{totalSeasonsInt}", exist_ok=True)
                    totalSeasonsInt = totalSeasonsInt - 1

                print(f"{Fore.GREEN}Created folders for {showTitle}{Style.RESET_ALL}")
                time.sleep(1)
                break

        elif tvoptions[menu_entry] == "[n] Add New":
            clear_screen()
            print("Name of show?")
            showName = input("Enter show name: ")
            os.makedirs(f"{mediapath}/tvshow/{showName}", exist_ok=True)
            print(f"{showName} Added!")
            time.sleep(1)

        else:
            clear_screen()
            options2 = ["[s] Seasons", "[r] Rename", "[d] Download"]
            choicemenu = TerminalMenu(options2, title=f"{tvoptions[menu_entry]} Options")
            indexthing = choicemenu.show()

            if indexthing is None:
                continue

            if options2[indexthing] == "[s] Seasons":
                clear_screen()
                seasons = os.listdir(f"{mediapath}/tvshow/{tvoptions[menu_entry]}")
                for season1 in seasons:
                    print(f"{season1}")

                seasonCount = int(input("Enter number of seasons: "))

                while seasonCount != 0:
                    os.makedirs(f"{mediapath}/tvshow/{tvoptions[menu_entry]}/Season_{seasonCount}", exist_ok=True)
                    seasonCount = seasonCount - 1

                print("Seasons Added!")
                recount = os.listdir(f"{mediapath}/tvshow/{tvoptions[menu_entry]}")
                print(recount)
                time.sleep(1)

            elif options2[indexthing] == "[r] Rename":
                print(f"What do you want to rename {tvoptions[menu_entry]} to?")
                rename = input("Enter Input: ")
                shutil.move(f"{mediapath}/tvshow/{tvoptions[menu_entry]}", f"{mediapath}/tvshow/{rename}")
                print(f"New name is {rename}")
                time.sleep(1)
                clear_screen()

            elif options2[indexthing] == "[d] Download":
                selected_show = tvoptions[menu_entry]
                while True:
                    clear_screen()
                    showDir = os.listdir(f"{mediapath}/tvshow/{selected_show}")

                    seasonsmenuoptions = ["[q] Quit"]

                    for dir in showDir:
                        seasonmediacheck = os.listdir(f"{mediapath}/tvshow/{selected_show}/{dir}")
                        combined = '\t'.join(seasonmediacheck)
                        mediafiletypes = ["mkv", "mp4", "mov", "avi", "wmv", "webm", "3gp"]

                        for mediatype in mediafiletypes:
                            if mediatype in combined:
                                seasonsmenuoptions.append(f"{dir} Media Files Found")
                                break
                        else:
                            seasonsmenuoptions.append(dir)

                    seasons_menu = TerminalMenu(seasonsmenuoptions)
                    entry = seasons_menu.show()

                    if entry is None:
                        break

                    if seasonsmenuoptions[entry] == "[q] Quit":
                        break

                    seasonentry = seasonsmenuoptions[entry].replace("Media Files Found", "").strip()

                    magnetlink = input("Enter Magnet Link: ")

                    headers = {
                        "Authorization": f"Bearer {apikey2}",
                        "Accept": "application/json"
                    }

                    if magnetlink:
                        rd_add_and_download(
                            magnetlink, headers,
                            f"{mediapath}/tvshow/{selected_show}/{seasonentry}/",
                            background=True
                        )

                    clear_screen()


def movies():
    links = []
    while True:
        print(f"[{Fore.GREEN}q{Style.RESET_ALL}] to go back.")
        magnetlink = input("Enter Magnet Link: ")
        print(magnetlink)
        clear_screen()

        if magnetlink not in ("quit", "done", "q"):
            links.append(magnetlink)
        print(links)

        headers = {
            "Authorization": f"Bearer {apikey}",
            "Accept": "application/json"
        }

        if magnetlink == "q":
            print(f"{Fore.GREEN}Going back to main menu...{Style.RESET_ALL}")
            time.sleep(0.5)
            clear_screen()
            return

        if magnetlink == "done":
            while links:
                url = links.pop(0)
                rd_add_and_download(url, headers, f"{mediapath}/movies/")
                clear_screen()
            return

        if magnetlink == "quit":
            clear_screen()
            print("Quitting...")
            return


def choose():
    while True:
        clear_screen()

        result = pyfiglet.figlet_format("Mediatool V2")
        print(Fore.GREEN + result)
        print(Style.RESET_ALL)

        options = [
            "[m] Movies",
            "[t] TV Shows",
            "[s] Search",
            "[e] Edit Config",
            "[q] Quit"
        ]

        terminal_menu = TerminalMenu(
            options,
            title="┌──────────────────────────┐\n"
                  "│        Main Menu         │\n"
                  "└──────────────────────────┘",
            menu_cursor=">",
            cycle_cursor=True
        )
        menu_entry_index = terminal_menu.show()

        if menu_entry_index is None:
            continue

        selection = options[menu_entry_index]

        print(f"You have selected: {selection}")

        if selection == "[e] Edit Config":
            setup()

        elif selection == "[m] Movies":
            print(Fore.GREEN + "You chose Movies!")
            time.sleep(0.5)
            print(Style.RESET_ALL)
            clear_screen()
            options = ["Manual", "RARBG Scrape"]
            moviechoose = TerminalMenu(options, title="Choose movie download type")
            menu_entry_index2 = moviechoose.show()
            if menu_entry_index2 is None:
                continue
            selection = options[menu_entry_index2]
            if selection == "Manual":
                clear_screen()
                movies()
            elif selection == "RARBG Scrape":
                clear_screen()
                scrape()

        elif selection == "[t] TV Shows":
            print(Fore.GREEN + "You chose TV Shows!")
            time.sleep(0.5)
            print(Style.RESET_ALL)
            clear_screen()
            tvshow()

        elif selection == "[s] Search":
            clear_screen()
            options = ["[m] Movies", "[q] Quit"]
            choosey = TerminalMenu(options, title="Search for stuff in media library")
            menu_entry3 = choosey.show()
            if menu_entry3 is None:
                continue
            selection = options[menu_entry3]
            if selection == "[m] Movies":
                clear_screen()
                moviesearch()

        else:
            print(f"{Fore.RED}Quitting!{Style.RESET_ALL}")
            time.sleep(1)
            clear_screen()
            exit()


choose()
